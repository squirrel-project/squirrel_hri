...
int people2D_engine::segmentscanJDC(int lidx, std::vector<LSL_Point3D_container> &clusters)
{
	...
	while(split_complete)
	{
		...
		{
			//~ max distance check
			int breaking_idx = (laserscan[lidx].data.pts, last_breaking_idx, params.sqjumpdist);
...
int people2D_engine::segmentscanJDC(laserscan_data &scan, std::vector<LSL_Point3D_container> &clusters)
{
        //~ bailing var
        char split_complete = 1;

        //~ numpts
        int ptsz = scan.data.pts.size();
        ...
        while(split_complete)
        {
                ...
                {
                        //~ max distance check
                        int breaking_idx = get_breakpoint(scan.data.pts, last_breaking_idx, params.sqjumpdist);
                        ...
                        if(breaking_idx - last_breaking_idx >= L_MINCLUSTERSZ)
                        {
                                ...
                                //~ pump it into
                                single_cluster.pts.insert(single_cluster.pts.begin(), scan.data.pts.begin() + last_breaking_idx, scan.data.pts.begin() + breaking_idx);
                                clusters.push_back(single_cluster);
                        }       
...
void people2D_engine::detect_people(laserscan_data &scan, std::vector<LSL_Point3D_container> &people)
{
        std::vector < std::vector <Real> > descriptor;
        std::vector<LSL_Point3D_container> clusters;
