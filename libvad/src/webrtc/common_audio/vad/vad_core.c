...
static int16_t GmmProbability(VadInstT* self, int16_t* features,
  ...
  if (total_power > self->threshold) {
    ...
    for (channel = 0; channel < kNumChannels; channel++) {
      ...
      // Calculate the log likelihood ratio: log2(Pr{X|H0} / Pr{X|H1}).
      // Approximation:
      // log2(Pr{X|H0} / Pr{X|H1}) = log2(Pr{X|H0}*2^Q) - log2(Pr{X|H1}*2^Q)
      //                           = log2(h0_test) - log2(h1_test)
...
int WebRtcVad_InitCore(VadInstT* self) {
  ...
  //threshold for minimum energy to trigger VAD
  self->threshold = 100;
...
VadResultT WebRtcVad_CalcVad48khz(VadInstT* inst, const int16_t* speech_frame,
  ...
  return WebRtcVad_CalcVad8khz(inst, speech_nb, frame_length / 6);
                           ...
                           size_t frame_length)
{
    ...
    // Do VAD on an 8 kHz signal
    return WebRtcVad_CalcVad8khz(inst, speechNB, len);
}
                          ...
                          size_t frame_length)
{
    ...
    VadResultT output;
    output.vad = inst->vad;
    output.total_energy = total_power;
    return output;
}
