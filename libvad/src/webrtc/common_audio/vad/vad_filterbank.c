...
static void LogOfEnergy(int16_t kMinEnergy, const int16_t* data_in, size_t data_length,
  ...
  *log_energy += offset;
  ...
  // With this code, it always gives bigger total energy than kMinEnergy
...
int16_t WebRtcVad_CalculateFeatures(VadInstT* self, const int16_t* data_in,
  ...
  LogOfEnergy(kMinEnergy, hp_60, length, kOffsetVector[5], &total_energy, &features[5]);
  //printf("total energy 3000 - 4000: %d", total_energy);
  // Energy in 2000 Hz - 3000 Hz.
  LogOfEnergy(kMinEnergy, lp_60, length, kOffsetVector[4], &total_energy, &features[4]);
 // printf("total energy 2000 - 3000: %d", total_energy);
  ...
  length >>= 1;  // |data_length| / 4 <=> bandwidth = 1000 Hz.
  LogOfEnergy(kMinEnergy, hp_60, length, kOffsetVector[3], &total_energy, &features[3]);
 // printf("total energy 1000 - 2000: %d", total_energy);
  ...
  LogOfEnergy(kMinEnergy, hp_120, length, kOffsetVector[2], &total_energy, &features[2]);
 // printf("total energy 500 - 1000: %d", total_energy);
  ...
  LogOfEnergy(kMinEnergy, hp_60, length, kOffsetVector[1], &total_energy, &features[1]);
 // printf("total energy 250 - 500: %d", total_energy);

  // Remove 0 Hz - 80 Hz, by high pass filtering the lower band.
  HighPassFilter(lp_60, length, self->hp_filter_state, hp_120);

  // Energy in 80 Hz - 250 Hz.
  LogOfEnergy(kMinEnergy, hp_120, length, kOffsetVector[0], &total_energy, &features[0]);
 // printf("total energy 80 - 250: %d", total_energy);
